...
#define TURBO 0x05
#define FLYWHEEL 0x93
#define FLYWHEELFULL 0x94
#define TARGET 0x90
...
void loop() {
  if ((commandByte == ALLIANCE_BLUE) &&
      ...
      (payloadByte2 == 0x26))
  { //blue alliance
    for (lead = 0; lead <= 13; lead++) {
      strip.setPixelColor(lead, blue);
      strip.show();
    }
    for (lead = 84; lead <= 98; lead++) {
      strip.setPixelColor(lead, blue);
      strip.show();
    }
    x = 2
...
else if ((commandByte == FLYWHEELFULL) &&
         (payloadByte1 == 0x95) &&
         (payloadByte2 == 0x99))
{
